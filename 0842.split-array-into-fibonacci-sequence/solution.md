...
func dfs(s []rune, pos int, ans *[]int) bool {
    ...
    // Get the MxtInt32, it works on local machine for
    // big strings, but on LeetCode should change to Int32.
	...
	for i := pos; i < min(pos+maxLen, len(s)); i++ {
        ...
        // If it's bigger than Maximum break.
		if num > int(MaxInt32) {
			break
		}

        // Check our temp answers.
		if len(*ans) >= 2 {
